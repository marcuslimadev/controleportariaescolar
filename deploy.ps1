...
try {
  ...
  $cmd = "if [ -f '$RemotePath/config/config.php' ]; then cp '$RemotePath/config/config.php' /tmp/scp-config.php; fi && rm -rf /tmp/scp-uploads && mkdir -p /tmp/scp-uploads && if [ -d '$RemotePath/public_html/uploads' ]; then cp -a '$RemotePath/public_html/uploads/.' /tmp/scp-uploads/; fi && if [ -d '$RemotePath/public/uploads' ]; then cp -a '$RemotePath/public/uploads/.' /tmp/scp-uploads/; fi && mkdir -p '$RemotePath/public_html' && find '$RemotePath/public_html' -mindepth 1 -maxdepth 1 -exec rm -rf {} + && rm -rf '$RemotePath/includes' '$RemotePath/database' '$RemotePath/scripts' '$RemotePath/config' && tar -xzf /tmp/scp-deploy.tar.gz -C '$RemotePath' && if [ -f /tmp/scp-config.php ]; then mv /tmp/scp-config.php '$RemotePath/config/config.php'; fi && cd '$RemotePath' && php scripts/migrate.php && cp -a '$RemotePath/public/.' '$RemotePath/public_html/' && mkdir -p '$RemotePath/public_html/uploads' && cp -a /tmp/scp-uploads/. '$RemotePath/public_html/uploads/' && rm -rf '$RemotePath/public' /tmp/scp-deploy.tar.gz /tmp/scp-uploads && chmod -R u=rwX,go=rX '$RemotePath'"
