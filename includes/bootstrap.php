...
function layout_header(string $title): void {
    ...
    if (!empty($_SESSION['user_id']) || !empty($_SESSION['responsavel_id'])) echo '<nav class="navbar navbar-dark bg-primary"><div class="container"><a class="navbar-brand brand-lockup" href="'.e(url(portal_home())).'"><img src="'.e(asset_url('assets/porta-aberta-logo.jpg')).'" alt="'.e(app_name()).'"><span>'.e(app_name()).'</span></a><div class="d-flex gap-2 align-items-center"><a class="btn btn-outline-light btn-sm lang-button" href="'.e(lang_url(current_locale()==='en'?'pt':'en')).'">'.e(current_locale()==='en'?'PT':'EN').'</a><a class="btn btn-outline-light btn-sm logout-button" href="'.e(url('logout.php')).'">'.e(t('logout')).'</a><button class="mobile-menu-button" type="button" aria-label="Abrir menu" aria-controls="app-menu" aria-expanded="false"><span></span><span></span><span></span></button></div></div></nav>'.portal_nav_html();
    echo '<main class="container pb-5">'; if ($flash) echo '<div class="alert alert-'.e($flash[1]).'">'.e($flash[0]).'</div>';
}
function layout_footer(): void { echo '</main><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script><script nonce="'.e(csp_nonce()).'">const menuButton=document.querySelector(".mobile-menu-button"),menu=document.querySelector("#app-menu"),overlay=document.querySelector(".app-menu-overlay"),closeMenu=document.querySelector(".app-menu-close");function setMenu(open){document.body.classList.toggle("menu-open",open);if(menuButton)menuButton.setAttribute("aria-expanded",open?"true":"false");if(menu)menu.setAttribute("aria-hidden",open?"false":"true")}if(menuButton)menuButton.addEventListener("click",()=>setMenu(!document.body.classList.contains("menu-open")));if(overlay)overlay.addEventListener("click",()=>setMenu(false));if(closeMenu)closeMenu.addEventListener("click",()=>setMenu(false));document.addEventListener("keydown",event=>{if(event.key==="Escape")setMenu(false)});document.querySelectorAll("#app-menu a").forEach(link=>link.addEventListener("click",()=>setMenu(false)));if("serviceWorker" in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("'.e(url('sw.js')).'"));}</script></body></html>'; }
