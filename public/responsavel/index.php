...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="historico-filhos-' . $from . '-a-' . $to . '.csv"');
    echo "\xEF\xBB\xBF";
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Aluno', 'Turma', 'Movimento', 'Responsável', 'Data e hora'], ';');
    foreach ($rows as $r) {
        if (empty($r['tipo'])) continue;
        fputcsv($out, [
            $r['aluno'] ?? '',
            $r['turma'] ?? '',
