...
<form method="post" action="<?=e(url('admin/post-save.php'))?>" enctype="multipart/form-data">
  ...
  <div class="row g-3">
    ...
    <div class="col-md-6"><label class="form-label fw-bold">Imagem</label><input type="file" class="form-control" name="imagem" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp"><?php if($post['imagem_url']):?><small class="text-muted">Imagem atual mantida se nenhuma nova for enviada.</small><?php endif?></div>
    <div class="col-md-6"><label class="form-label fw-bold">Anexo</label><input type="file" class="form-control" name="anexo" accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp"><?php if($post['anexo_url']):?><small class="text-muted">Anexo atual: <a href="<?=e($post['anexo_url'])?>" target="_blank" rel="noopener"><?=e($post['anexo_nome'] ?: 'Abrir anexo')?></a>. Será mantido se nenhum novo for enviado.</small><?php else:?><small class="text-muted">Use para circulares, cardápios, autorizações ou imagens de apoio.</small><?php endif?></div>
