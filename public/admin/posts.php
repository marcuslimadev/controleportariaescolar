...
<tr><td><strong><?=e($post['titulo'])?></strong><br><small><?=e($post['turma'] ?: $post['aluno'] ?: ($post['publico']==='publico' ? 'Página inicial pública' : 'Todos os usuários/equipe'))?></small></td><td><?=e($post['tipo'])?></td><td><?=e($scopeLabels[$post['publico']] ?? $post['publico'])?></td><td><span class="status-pill <?=$post['status']==='publicado'?'active':'inactive'?>"><?=e($post['status'])?></span></td><td><?=e($post['autor'])?></td><td class="text-end"><div class="post-actions"><a class="btn btn-sm btn-outline-primary" href="<?=e(url('admin/post-form.php?id='.(int)$post['id']))?>">Editar</a><form method="post" action="<?=e(url('admin/post-delete.php'))?>" class="js-confirm-delete"><input type="hidden" name="csrf" value="<?=e(csrf())?>"><input type="hidden" name="id" value="<?=(int)$post['id']?>"><button class="btn btn-sm btn-outline-danger" type="submit">Excluir</button></form></div></td></tr>
<?php endforeach ?>
</tbody></table></div></div>
